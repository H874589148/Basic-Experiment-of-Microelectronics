puts "RM-Error: This top-level script is for a hierarchical flow only.  To use this script, please regnerate the RM scripts with Hierarchical Flow set to TRUE"
exit
...
if { ${DC_BLOCK_ABSTRACTION_DESIGNS} != ""} {
  remove_design -hierarchy ${DC_BLOCK_ABSTRACTION_DESIGNS}
}

if { ${DC_BLOCK_ABSTRACTION_DESIGNS_TIO} != ""} {
  remove_design -hierarchy ${DC_BLOCK_ABSTRACTION_DESIGNS_TIO}
}
...
# Remove the hierarchical designs 
if {![shell_is_in_topographical_mode]} {
  if { ${DDC_HIER_DESIGNS} != "" } {
    remove_design -hierarchy ${DDC_HIER_DESIGNS}
  }
...
# Write out mapped top-level design
